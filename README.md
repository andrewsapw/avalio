...
```bash
$ git clone https://github.com/andrewsapw/avalio.git
$ cd avalio
$ go build .
```
...
retry_delay = 2   # Optional: configure delay between retry attempts in seconds
...
Для каждого HTTP-ресурса можно настроить параметры `max_retries` и `retry_delay`:
- `max_retries` определяет количество попыток повторной проверки ресурса при первоначальной неудаче. Если параметр не указан, используется значение по умолчанию - 3 попытки.
- `retry_delay` задает интервал между повторными попытками проверки ресурса в секундах. Если параметр не указан, используется значение по умолчанию - 1 секунда.
